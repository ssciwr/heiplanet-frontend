...
export class GridProcessingStore {
	...
	private prevFirstDatapointModelValue: number | undefined = undefined;
	...
	generateAdaptiveGridCells = (
		...
		const filteredData = dataPoints
			.filter(
				(point: DataPoint) =>
					...
					Number.isFinite(point.modelValue),
			)
			.filter(
				(point: DataPoint) =>
		...
		for (const point of filteredData) {
			...
			if (cellMap.has(cellId)) {
				...
				if (cell) {
					cell.sum += point.modelValue;
					cell.count += 1;
				}
			} else {
				cellMap.set(cellId, {
					sum: point.modelValue,
					count: 1,
					bounds,
				});
			}
		...
		const result = Array.from(cellMap.entries()).map(([id, data]) => ({
			id,
			bounds: data.bounds,
			modelValue: data.sum / data.count,
		}));
		console.log(
	...
	generateGridCellsFromModelOutputData = (
		outputData: ModelOutputDataPoint[],
		viewport: ViewportBounds | null,
		resolutionLevel: number,
	): GridCell[] => {
		const methodStart = performance.now();
		console.log(
			"🚀 generateGridCellsFromModelOutputData START with:",
			outputData.length,
			"points, resolution:",
			resolutionLevel,
		);

		if (!viewport || !outputData.length) {
			console.log(
				"⚠️ Early exit - no viewport or model output data",
				!!viewport,
				outputData.length,
			);
			return [];
		...
		const currentFirstDatapointModelValue = outputData[0]?.modelValue;
		...
		const hasDataChange =
			this.prevFirstDatapointModelValue !== currentFirstDatapointModelValue;
		...
		if (hasSignificantViewportChange || hasResolutionChange || hasDataChange) {
			...
			console.log(
				`✅ generateGridCellsFromModelOutputData COMPLETE in ${methodTotal.toFixed(2)}ms`,
			);
			return cells;
		}
